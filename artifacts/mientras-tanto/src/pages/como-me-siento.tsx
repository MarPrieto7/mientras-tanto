...
type EmotionState =
  | "saturado" | "triste" | "vacio"
  | "cansado" | "sensible" | "confundido" | "en calma" | null;
...
export default function ComoMeSiento() {
  ...
  const { playEmotion, stopEmotion, activeEmotionId } = useMusic();

  // Restore previously selected emotion
  const [selected, setSelected] = useState<EmotionState>(() => {
    return (localStorage.getItem("mientras_tanto_emotion") as EmotionState) || null;
  });

  // On mount: if emotion was already selected, resume its audio
  useEffect(() => {
    if (selected && activeEmotionId !== selected) {
      playEmotion(selected);
    }
  ...
  const handleBreathing = () => {
    // Emotion audio continues while in breathing mode
    setLocation("/respiracion");
  };
            ...
            >
              ...
              <div className="relative w-40 h-40 mb-6 flex items-center justify-center">
                <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"
                  ...
                  <path fill="currentColor" d={blobs[selected]}
