...
import { emotionProfiles, EmotionId } from "@/lib/ambient";
...
export default function ModoRespiracion() {
  ...
  const { activeEmotionId } = useMusic();

  const phaseIndexRef  = useRef(0);
  const timerRef       = useRef<ReturnType<typeof setTimeout> | null>(null);
  const countdownRef   = useRef<ReturnType<typeof setInterval> | null>(null);

  // Get breath cycle from emotion profile (or neutral fallback)
  const savedEmotion = (activeEmotionId || localStorage.getItem("mientras_tanto_emotion")) as EmotionId | null;
  ...
  }, [inhale, hold, exhale]); // eslint-disable-line react-hooks/exhaustive-deps

  const startSession = () => {
    setStarted(true);
    phaseIndexRef.current = 0;
    runPhase(0);
    // Emotion MP3 from MusicContext keeps playing — no new audio started here
  };

  const handleClose = () => {
    if (timerRef.current)    clearTimeout(timerRef.current);
    if (countdownRef.current) clearInterval(countdownRef.current);
    // Emotion audio continues until user goes back in como-me-siento
  ...
  const circleScale   = phase === "inhala" || phase === "sostén" ? 1.6 : 1.0;
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-foreground-soft text-sm leading-loose max-w-xs font-sans">
              La música y el movimiento guiarán tu respiración.
              <br />
